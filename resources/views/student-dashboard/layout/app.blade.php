...
<aside>
    <div class="side-logo">
        ...
        <div class="sideButton">
            <div class="mobil-button">
                <button class="navbar-toggler open collapsed" >
                    <span></span>
                    <span></span>
                    <span></span>
                </button>
    ...
    <ul>
        @php
            $educations = array();
       $student_educations = \App\Models\StudentEducations::where('student_id',\Illuminate\Support\Facades\Auth::user()->id)->get();
       foreach ($student_educations as $education){
           $edu = \App\Models\Education::where('id',$education->education_id)->first();
           array_push($educations,$edu);
       }@endphp
        @foreach($educations as $education)
            <li><a href="{{route('student.week',$education->id)}}"><i class="fa-solid fa-book"></i></i><span>{{$education->title}}</span></a></li>
        @endforeach
    </ul>
</aside>
<main>
    <nav>
        <h2><b>@yield("title1")</b>@yield("title2")</h2>
        <div>
            <a href=""><i class="fa-regular fa-bell"></i></a>
            <a href="{{route('education.logout')}}"><i class="fa-light fa-arrow-right-from-bracket"></i></a>
            <a href="">
                <img src="{{asset("front/img/default-male.jpg")}}" alt="">
            </a>
        </div>
    </nav>
    <div id="main">
...
<script>
    var array = $("input");
    array.each(function (input){
        if(array[input].value){
            array[input].parentElement.classList.add("valid")
        }
        else{
            array[input].parentElement.classList.remove("valid")
        }
    })
    $("input").keyup(function (event){
        if(event.target.value){
            event.currentTarget.parentElement.classList.add("valid")
        }
        else{
            event.currentTarget.parentElement.classList.remove("valid")
        }
    })
</script>
<script>
    ...
    textarea.each(function (input){
        if(textarea[input].value){
            textarea[input].parentElement.classList.add("valid")
        }
        else{
            textarea[input].parentElement.classList.remove("valid")
        }
    })
